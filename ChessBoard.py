...
class Board:
	...
	def checkEnPassant(self, pawn, pawnPieceList, opponentPieceList):
		potentialSpace = self.doubleUpPawn.position[0]-1, self.doubleUpPawn.position[1]
...
class Pawn(Piece):
	...
	def updatePotentialSpaces(self, myPieceList, opponentPieceList, checkMyKing):
		...
		if checkMyKing: #checking pawn from its player's perspective
			moveDiagonals = [(self.row-1, self.col+1), (self.row-1, self.col-1)]
		else:			#checking pawn from opponent's perspective
			moveDiagonals = [(self.row+1, self.col+1), (self.row+1, self.col-1)]
